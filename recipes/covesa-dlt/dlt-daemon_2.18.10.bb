...
DEPENDS = "zlib gzip-native json-c logd"
...
PACKAGECONFIG ?= "${@bb.utils.contains('DISTRO_FEATURES', 'systemd', ' systemd systemd-watchdog systemd-journal ', '', d)} \
 ...
 udp-connection dlt-system dlt-filetransfer dlt-logd-converter"
# dlt-dbus
...
PACKAGECONFIG[dlt-logd-converter] = "-DWITH_DLT_ADAPTOR=ON,-DWITH_DLT_ADAPTOR=OFF"
...
EXTRA_OECMAKE += "-DWITH_DLT_LOGSTORAGE_GZIP=ON -DWITH_EXTENDED_FILTERING=ON -DSYSTEMD_UNITDIR=${systemd_system_unitdir} \
                  -DWITH_DLT_USE_IPv6=OFF -DWITH_DLT_SYSTEM=ON -DWITH_DLT_ADAPTOR=ON -DDLT_IPC='UNIX_SOCKET'"
...
SYSTEMD_SERVICE:${PN} = " ${@bb.utils.contains('PACKAGECONFIG', 'systemd', 'dlt.service', '', d)} \
                          ...
                          ${@bb.utils.contains('PACKAGECONFIG', 'systemd dlt-logd-converter', 'dlt-logd-converter.service', '', d)} \
...
do_install:append() {
    ...
    install -m 0744 ${WORKDIR}/dlt-system.conf ${D}${sysconfdir}/dlt-system.conf
    install -m 0644 ${WORKDIR}/dlt-logd-converter.service ${D}${systemd_unitdir}/system/dlt-logd-converter.service
}
